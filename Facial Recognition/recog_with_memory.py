...
def main():
    mem = MemoryDB("theradog_memory.sqlite")  # ok to use any path
    ...
    while True:
        ...
        if key == ord('e'):
            if faces:
                ...
                mem.ensure_person(OWNER_NAME)  # no return value
        ...
        for face in faces:
            ...
            face_crop = crop_face(frame, face.bbox, expand=0.18)
            try:
            ...
            if best_name != "Unknown":
                person_id = best_name                 # use name as ID
                mem.ensure_person(person_id)          # make sure person exists
                profile = mem.get_profile(person_id)  # has pref_angle_deg

                episode = {
                    "ts": time.time(),
                    "distance": profile["pref_distance"],
                    "angle_deg": profile["pref_angle_deg"],
                ...
                mem.upsert_episode(person_id, episode)
                mem.update_profile_from_episode(person_id, episode)
